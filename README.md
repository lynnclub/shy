...
**本框架目前是初始版本，在不断的优化、完善之中，而且后续更新可能涉及大范围改动，仅供学习参考。**
...
本框架是以容器为基础的，就连框架本身都在容器里面，由此为开发者提供了一个自由度极高的实例复用池。在容器的基础上，框架核心提供了异常处理、门面、流水线等实用特性。在框架核心的基础之上，提供了web服务、终端服务、socket服务等。

本框架除了支持传统的web服务运行方式，还支持**常驻内存运行模式**、即进程复用不销毁。**在该模式下，由于避免了重复编译导致的消耗，性能相对于传统web方式大幅度提升。**
...
该函数运行时，如果容器内已经存在待加入的抽象名称，此时不会做加入操作，而是直接返回该抽象名称对应的旧实例。如果不存在，才会将该抽象名称及其对应的实例加入到容器中，并返回被加入的实例。
...
**由上述内容可见，本框架为开发者提供了自由的开放容器、并且包括框架核心在内的实例都在容器里面，可以很方便地操作。这提升了开发自由度，但是，开发者在操作框架核心类的时候，一定要仔细梳理逻辑，以免产生问题。**

### 3. 异常、错误处理

本框架在终端服务、http服务等入口处注册了异常或错误处理，能够捕获到全局的异常或错误。Exception、Error、ErrorException、甚至是shutdown，都会被捕捉处理到。

根据服务的不同，需要不同的错误处理方法，框架为每个服务提供了handler。你也可以实现handler接口来自定义错误处理，再通过修改服务入口的handler使其生效。
...
门面类的`getInstance()`方法没有被重写时会抛出异常：
...
本框架可使用phpunit做单元测试。在tests文件夹中编写测试代码，在框架根目录输入下面的命令即可执行测试。
...
常驻内存模式是以CLI方式执行web框架。该模式相对与传统的web服务执行方式，做到了进程复用不销毁，避免了代码重复编译导致的消耗，所以框架性能得到大幅度提升。

本框架基于WorkerMan提供的socket服务，实现了兼容命令模式运行、常驻内存的web服务框架。即传统web模式与常驻内存模式可以同时运行。
...
cli运行环境相对于传统web运行环境有根本差异，所以有很多需要注意的地方。**其中，最需要注意的就是实例的循环复用。所以，在实例复用之前必须恢复初始状态，否则就会出现带着 上一个请求的状态 执行新的请求，这种混乱的情况**。框架本身已经做好了初始化处理，但是框架无法顾及到开发者实现的、业务逻辑的部分。

开发者注意事项：

1. 业务逻辑实例如果想要复用，必须做好初始化，否则会出现混乱。如果不需要复用，在常驻内存模式下运行请销毁实例。
2. 需要改变的值不能用常量，否则复用的时候无法重新赋值。
3. header函数在socket环境下不能正常使用，可以使用WorkerMan的http对象的方法。但是echo、var_dump或者页面等可以正常输出，因为框架做了ob缓冲区、会自动把缓冲区内容放在WorkerMan的通道里面输出。
